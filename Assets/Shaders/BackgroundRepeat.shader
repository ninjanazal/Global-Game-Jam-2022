...
Shader "GGJ/VoronoiShader"
{
    Properties
    {
		_TargetTexture("Defined Texture", 2D) = "white" {}
		[MaterialToggle] _useTarget("Use define texture", float) = 0
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			sampler2D _TargetTexture; // Target texture
            ...
            fixed4 frag (v2f i) : SV_Target
            {
				...
				if(_useTarget == 0){
					...
					grabColor = tex2Dproj(_GrabTexture, UNITY_PROJ_COORD(i.uv + (voronoiDisplace.x * _Amount)));
				}
				else {
					// pega no valor da cor de frag na textura de grab, influenciado pelo voronoi e a intensidade
					grabColor = tex2Dproj(_TargetTexture, UNITY_PROJ_COORD(i.uv + (voronoiDisplace.x * _Amount)));
				}
